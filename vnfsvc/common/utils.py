...
from vnfsvc.openstack.common import lockutils
from vnfsvc.openstack.common import log as logging


TIME_FORMAT = "%Y-%m-%dT%H:%M:%SZ"
LOG = logging.getLogger(__name__)
SYNCHRONIZED_PREFIX = 'vnfsvc-'
...
def find_config_file(options, config_file):
    ...
    * Search for the configuration files via common cfg directories
    ...
    config_file_dirs = [fix_path(os.path.join(os.getcwd(), 'etc')),
                        fix_path(os.path.join('~', '.vnfsvc-venv', 'etc',
                                              'vnfsvc')),
                        fix_path('~'),
                        os.path.join(cfg.CONF.state_path, 'etc'),
                        os.path.join(cfg.CONF.state_path, 'etc', 'vnfsvc'),
                        fix_path(os.path.join('~', '.local',
                                              'etc', 'vnfsvc')),
                        '/usr/etc/vnfsvc',
                        '/usr/local/etc/vnfsvc',
                        '/etc/vnfsvc/',
                        '/etc']

    if 'plugin' in options:
        config_file_dirs = [
            os.path.join(x, 'vnfsvc', 'plugins', options['plugin'])
